...
test.each([['with stable connection'], ['without stable connection']])(
  ...
  async scenario => {
    ...
    Date.now = () => originalDateNow() + dateOffsetMs;
    ...
    try {
      ...
      expect(connectingTime - disconnectTime).toBeLessThan(retryDelayMs);

      if (scenario === 'with stable connection') {
        // THEN: Should reconnect immediately.
        expect(onUpgrade).toHaveBeenCalledTimes(2);
        expect(onUpgrade.mock.results[1].value - disconnectTime).toBeLessThan(retryDelayMs);
      } else {
        // THEN: Should reconnect after the retry delay.
        expect(onUpgrade).toHaveBeenCalledTimes(2);
        expect(onUpgrade.mock.results[1].value - disconnectTime).toBeGreaterThanOrEqual(retryDelayMs);
        expect(onUpgrade.mock.results[1].value - disconnectTime).toBeLessThanOrEqual(
